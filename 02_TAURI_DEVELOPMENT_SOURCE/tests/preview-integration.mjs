...
assert.equal(listDocument.querySelectorAll(".notice-list-item").length, 4, "공고 선택 화면에는 공고 카드 4개가 표시되어야 합니다.");
...
assert.match(document.querySelector(".source-line").textContent, /공식 공고 내용을 바탕으로 준비한 예시 답변/, "공식 공고 기반 예시 답변임을 알려야 합니다.");
assert.match(document.querySelector(".source-line").textContent, /실제 AI 답변이 아닙니다/, "실제 AI 결과가 아님을 알려야 합니다.");
...
assert.equal(document.querySelector(".example-badge").textContent, "예시 답변", "결과 카드에 예시 답변 표시가 있어야 합니다.");
...
console.log("preview integration: 35 checks passed");
