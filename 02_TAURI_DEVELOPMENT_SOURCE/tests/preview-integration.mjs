...
assert.match(document.querySelector("#notice-title").textContent, /2026학년도 비교과 프로그램 모집/, "가상 공고 제목이 표시되어야 합니다.");
...
assert.match(document.querySelector("#answer-copy").textContent, /7월 20일부터 7월 31일까지/, "FAQ에 맞는 예시 답변이 표시되어야 합니다.");
assert.match(document.querySelector("#answer-source").textContent, /공고문.*신청 기간/, "공고문 출처가 표시되어야 합니다.");
...
setValue(window, "#question-input", "편입생도 신청 가능한가요?");
submit(window);
assert.match(document.querySelector("#answer-copy").textContent, /편입생은 신청 가능합니다/, "정상 입력에 핵심 답변이 표시되어야 합니다.");
...
assert.match(document.querySelector(".department-details").textContent, /031-000-0000/, "가상 연락처가 표시되어야 합니다.");
...
console.log("preview integration: 25 checks passed");
