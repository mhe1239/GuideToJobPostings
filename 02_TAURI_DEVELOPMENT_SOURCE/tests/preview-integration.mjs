...
listWindow.KANGNAM_FIREBASE = {
  auth: {},
  roleLists: { owners: ["owner@kangnam.ac.kr"], editors: [] },
  onAuthStateChanged: (_auth, callback) => callback({ email: "owner@kangnam.ac.kr" }),
  signOut: async () => {
    headerSignOutCalled = true;
  },
};
...
assert.match(listDocument.querySelector("#header-account-menu").textContent, /관리자 메뉴/, "관리자 권한 계정에는 관리자 메뉴 항목이 보여야 합니다.");
assert.match(listDocument.querySelector("#header-account-menu").textContent, /로그아웃/, "계정 메뉴에는 로그아웃 항목이 있어야 합니다.");
listDocument.querySelector("#header-account-menu button").click();
await new Promise((resolve) => listWindow.setTimeout(resolve, 0));
assert.equal(headerSignOutCalled, true, "계정 메뉴의 로그아웃은 Firebase 로그아웃을 호출해야 합니다.");
