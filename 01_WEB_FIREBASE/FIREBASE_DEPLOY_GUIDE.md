...
이 폴더는 `강남대 공고 길잡이` FIXED 버전의 Firebase Hosting 배포용 파일입니다.

화면과 가상 예시 답변은 `app/` 폴더에 있으며 실제 AI, 로그인, 데이터베이스 또는 개인정보를 사용하지 않습니다.
...
완료 후 터미널에 표시되는 `https://...web.app` 주소를 검증자에게 전달합니다. Preview URL은 주소를 아는 누구나 접속할 수 있으므로 가상 데이터 검증용으로만 사용합니다.
